...
For a quick isolated test running could be used Docker environment with:

  docker run -v $(pwd):/home -w /home -it ruby:2.5.5-alpine3.9 sh -c 'apk add git; bundle install; rake test'

Where we can switch different ruby versions and platforms using different docker images.
If you want to get `pry` insights during test running it can be achieved by installing a few dependencies
and setting as usual breakpoint `binding.pry`:

  docker run -v $(pwd):/home -w /home -it ruby:2.6.2-alpine3.9 sh -c 'apk add git build-base gcc make; bundle install; gem install pry-byebug; rake test'
